...
- (void)updateFlashlightState {
  ...
  [self.flashButton setTitle:flashlightButtonTitle
...
- (void)addFlashButton {
  ...
  UIImage *flashButtonImage =
      [[UIImage imageNamed:@"JPSImagePickerController.bundle/flash_button"]
          imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
  [self.flashButton setImage:flashButtonImage forState:UIControlStateNormal];
  [self.flashButton addTarget:self
                       action:@selector(didPressFlashButton)
             forControlEvents:UIControlEventTouchUpInside];
  [self.view addSubview:self.flashButton];

  NSLayoutConstraint *left =
      [NSLayoutConstraint constraintWithItem:self.flashButton
                                   attribute:NSLayoutAttributeLeft
                                   relatedBy:NSLayoutRelationEqual
                                      toItem:self.view
                                   attribute:NSLayoutAttributeLeft
...
- (void)addCameraSwitchButton {
  ...
  [self.cameraSwitchButton
      setBackgroundImage:[UIImage imageNamed:@"JPSImagePickerController.bundle/"
                                  @"camera_switch_button"]
                forState:UIControlStateNormal];
  [self.cameraSwitchButton addTarget:self
                              action:@selector(didPressCameraSwitchButton)
                    forControlEvents:UIControlEventTouchUpInside];
  [self.view addSubview:self.cameraSwitchButton];

  NSLayoutConstraint *right =
      [NSLayoutConstraint constraintWithItem:self.cameraSwitchButton
...
- (void)operationCompleted {
  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self.session) return;
    self.capturePreviewView = [[UIView alloc]
        initWithFrame:CGRectOffset(self.capturePreviewLayer.frame, 0, 69.0f)];
#if TARGET_IPHONE_SIMULATOR
    self.capturePreviewView.frame =
        CGRectMake(0, 0, self.view.bounds.size.width,
                   self.view.bounds.size.height - 73.0f);
...
- (void)takePicture {
  ...
  [output captureStillImageAsynchronouslyFromConnection:
              videoConnection completionHandler:^(CMSampleBufferRef
    ...
    self.imageOrientation = [JPSImagePickerController
        currentImageOrientation:self.deviceOrientation];

    UIImage *image = [UIImage
        imageWithCGImage:[[[UIImage alloc] initWithData:imageData] CGImage]
                   scale:1.0f
...
- (void)didPressCameraSwitchButton {
  ...
  // avoid crash if the user spams the button
...
- (void)addPreview {
  ...
  self.previewImageView =
      [[UIImageView alloc] initWithFrame:self.capturePreviewView.bounds];
  self.previewImageView.contentMode = UIViewContentModeScaleAspectFit;
  self.previewImageView.image = self.previewImage;
  self.previewImageView.clipsToBounds = YES;

  UIScrollView *previewScrollView =
      [[UIScrollView alloc] initWithFrame:self.capturePreviewView.frame];
...
- (void)addRetakeButton {
  ...
  [self.retakeButton addTarget:self
                        action:@selector(retake)
              forControlEvents:UIControlEventTouchUpInside];
  [self.view addSubview:self.retakeButton];

  NSLayoutConstraint *left =
      [NSLayoutConstraint constraintWithItem:self.retakeButton
                                   attribute:NSLayoutAttributeLeft
                                   relatedBy:NSLayoutRelationEqual
                                      toItem:self.view
                                   attribute:NSLayoutAttributeLeft
...
- (void)addUseButton {
  ...
  NSLayoutConstraint *right =
      [NSLayoutConstraint constraintWithItem:self.useButton
                                   attribute:NSLayoutAttributeRight
                                   relatedBy:NSLayoutRelationEqual
                                      toItem:self.view
                                   attribute:NSLayoutAttributeRight
...
- (void)addConfirmationLabel {
  ...
  NSLayoutConstraint *centerX =
      [NSLayoutConstraint constraintWithItem:self.confirmationLabel
                                   attribute:NSLayoutAttributeCenterX
                                   relatedBy:NSLayoutRelationEqual
                                      toItem:self.view
                                   attribute:NSLayoutAttributeCenterX
...
- (void)addConfirmationOverlayLabel {
  ...
  self.confirmationOverlayLabel.backgroundColor =
      self.confirmationOverlayBackgroundColor;
  self.confirmationOverlayLabel.text = self.confirmationOverlayString;
  [self.view addSubview:self.confirmationOverlayLabel];

  NSLayoutConstraint *centerX =
      [NSLayoutConstraint constraintWithItem:self.confirmationOverlayLabel
                                   attribute:NSLayoutAttributeCenterX
                                   relatedBy:NSLayoutRelationEqual
                                      toItem:self.view
                                   attribute:NSLayoutAttributeCenterX
...
- (void)setConfirmationOverlayBackgroundColor:
    (UIColor *)confirmationOverlayBackgroundColor {
  _confirmationOverlayBackgroundColor = confirmationOverlayBackgroundColor;
  if (self.confirmationOverlayLabel) {
    self.confirmationOverlayLabel.backgroundColor =
        confirmationOverlayBackgroundColor;
  }
}
